...
|-------|--------------|--------|
| **Reachability** | Is the call path from entry to this function real? The attack path from Phase 1 trusted callgraph.json edges. Now you independently verify. **Direct edges**: confirm the callee name appears in the caller's source. **Indirect edges (any confidence)**: find the function pointer assignment (e.g., `ops->handler = &func`, `ctx->callback = handler`) and confirm it resolves to the callee. **Medium/low confidence indirect edges are the highest priority to verify** — Phase 1 accepted them without checking, so this is your primary gate. If you cannot find the function pointer assignment resolving to the claimed callee for a medium/low confidence indirect edge, that is a FAIL. | PASS / FAIL / UNCERTAIN |
